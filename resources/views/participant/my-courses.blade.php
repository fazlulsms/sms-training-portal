...
@php
    ...
    // "Continue Learning" spotlight — active, unlocked, has next lesson
    $continueLearning = $elearningEnrollments
        ->where('completion_status', '!=', 'completed')
        ->where('access_status', 'unlocked')
        ->filter(fn($e) => isset($nextLessonMap[$e->id]))
        ->take(2);

    // IDs shown in Continue Learning — exclude from the E-Learning grid
    $continueIds = $continueLearning->pluck('id');

    // Remaining e-learning courses (not in spotlight)
    $remainingEl = $elearningEnrollments->whereNotIn('id', $continueIds);
@endphp
...
/* ── Welcome Card ── */
.welcome-card {
    background: linear-gradient(135deg, #0f1e45 0%, #1e3a8a 60%, #2563eb 100%);
    border-radius: 16px;
    padding: 26px 28px;
    color: white;
    margin-bottom: 24px;
    ...
    flex-wrap: wrap;
    gap: 20px;
    box-shadow: 0 8px 24px rgba(15,30,69,.30);
    position: relative;
    overflow: hidden;
}
.welcome-card::before {
...
.welcome-text h2 { margin: 0 0 3px; font-size: 19px; font-weight: 800; }
.welcome-text p  { margin: 0; font-size: 13px; opacity: .8; }
.welcome-stats { display: flex; gap: 24px; flex-wrap: wrap; position: relative; z-index: 1; }
.ws-item { text-align: center; }
.ws-num  { font-size: 24px; font-weight: 800; line-height: 1; }
.ws-lbl  { font-size: 10.5px; opacity: .7; font-weight: 600; text-transform: uppercase; letter-spacing: .6px; margin-top: 2px; }
...
.sum-card {
    ...
    border: 1px solid #e5e7eb; box-shadow: 0 1px 6px rgba(15,23,42,.05);
...
.continue-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 18px; }
.cl-card {
...
.cl-banner { height: 6px; background: linear-gradient(90deg, #2563eb, #60a5fa); }
...
/* ── Certificate mini cards ── */
.cert-cards-row { display: flex; flex-wrap: wrap; gap: 14px; }
.cert-mini {
    background: #fff; border-radius: 12px; border: 1px solid #e5e7eb;
    padding: 14px 18px; display: flex; align-items: center; gap: 14px;
    flex: 1; min-width: 260px; box-shadow: 0 1px 4px rgba(15,23,42,.05);
}
...
.cc-meta-label { font-size: 10.5px; color: #9ca3af; font-weight: 700; text-transform: uppercase; letter-spacing: .5px; margin-bottom: 3px; }
.cc-meta-val   { font-size: 13px; font-weight: 700; color: #374151; }
...
@media (max-width: 600px) {
    ...
    .welcome-left { gap: 12px; }
    .welcome-avatar { width: 44px; height: 44px; font-size: 16px; }
    .welcome-text h2 { font-size: 16px; }
    .course-grid, .continue-grid { grid-template-columns: 1fr; }
}
</style>
...
<div class="welcome-card">
    <div class="welcome-left">
        ...
        <div class="welcome-text">
            <h2>Welcome back, {{ explode(' ', $user->name ?? 'Learner')[0] }} 👋</h2>
...
<div class="summary-grid">
    <a href="#all-courses" class="sum-card">
        ...
        <div><div class="sum-val">{{ $totalAll }}</div><div class="sum-lbl">My Courses</div></div>
    </a>
    <a href="#continue-learning" class="sum-card">
        <div class="sum-icon si-amber">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
        </div>
        <div><div class="sum-val">{{ $inProgressEl }}</div><div class="sum-lbl">In Progress</div></div>
    </a>
    <a href="#all-courses" class="sum-card">
        <div class="sum-icon si-green">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
        </div>
        <div><div class="sum-val">{{ $completedEl }}</div><div class="sum-lbl">Completed</div></div>
    </a>
    <a href="{{ route('participant.my-certificates') }}" class="sum-card">
        ...
        <div><div class="sum-val">{{ $certIssued }}</div><div class="sum-lbl">Certificates</div></div>
    </a>
</div>

{{-- ══ CONTINUE LEARNING (spotlight) ═══════════════════════ --}}
@if($continueLearning->isNotEmpty())
<div id="continue-learning" class="lms-section">
    <div class="lms-title">
...
{{-- ══ E-LEARNING COURSES (all except those in spotlight) ══ --}}
<div id="all-courses" class="lms-section">
    <div class="lms-title">
        ...
        @if($remainingEl->isEmpty() && $continueLearning->isNotEmpty())
    ...
    @if($elearningEnrollments->isEmpty())
        <div style="padding:32px;text-align:center;border:2px dashed #e5e7eb;border-radius:14px;background:#fafafa;color:#9ca3af;font-size:14px;">
            No e-learning enrollments yet. Contact admin to get enrolled.
        </div>
    @elseif($remainingEl->isEmpty() && $continueLearning->isNotEmpty())
        {{-- Only course(s) are already shown above in Continue Learning --}}
        <div style="padding:20px 24px;background:#fff;border:1px solid #e5e7eb;border-radius:14px;display:flex;align-items:center;gap:14px;color:#6b7280;font-size:14px;">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#2563eb" stroke-width="2"><polygon points="5 3 19 12 5 21 5 3"/></svg>
            <span>Your active course{{ $continueLearning->count() > 1 ? 's are' : ' is' }} shown in <strong style="color:#111827;">Continue Learning</strong> above.</span>
        </div>
    @else
        <div class="course-grid">
            @foreach($remainingEl as $enrollment)
            @php
                $pct          = $enrollment->progress_percentage;
