...
.bdr-navy {
    ...
    border: 2px solid #0f2055;
    z-index: 3;
}
/* Gold corner diamonds */
.co { position: absolute; width: 7mm; height: 7mm;
      border-color: #c9a227; border-style: solid; z-index: 4; }
...
/* ── Watermark: faint SMS logo behind content ─────── */
.wm {
    position: absolute;
    top: 50%; left: 50%;
...
.micro {
    position: absolute;
    top: 45mm; left: 10mm; right: 100mm;
    bottom: 28mm;
    font-size: 5px; color: #1a3a8a; opacity: 0.05;
    line-height: 5.5mm; word-break: break-all; z-index: 1;
    overflow: hidden;
}

/* ═══════════════════════════════════════════════════
   SECTION 1 — HEADER BAND   (0 → 42mm)
═══════════════════════════════════════════════════ */
.hdr {
    position: absolute;
...
/* Subtle 45° stripe overlay */
.hdr-stripe {
    position: absolute; top: 0; left: 0; right: 0; bottom: 0; opacity: 0.06;
    background-image: repeating-linear-gradient(
        45deg, #fff 0, #fff 1px, transparent 1px, transparent 11px);
}
/* Logo panel — left 68mm */
.hdr-logo-panel {
    position: absolute;
...
.hdr-logo-img {
    position: absolute;
    top: 50%; left: 50%;
    width: 50mm; margin-top: -10mm; margin-left: -25mm;
}
.hdr-logo-img img { height: 20mm; }

/* Title panel — 68mm → right */
.hdr-title-panel {
    position: absolute;
...
.rule {
    position: absolute;
    left: 0; right: 0; height: 2.5mm; z-index: 2;
}
.rule-top    { top: 42mm; }
...
/* ═══════════════════════════════════════════════════
   SECTION 2 — BODY  (44.5 → 159mm)
═══════════════════════════════════════════════════ */

/* Left body — main certificate text */
.body-left {
    position: absolute;
...
.completed-line {
    font-size: 9.5px; color: #4b5563; font-style: italic;
    margin-bottom: 2.5mm;
}
.cname {
...
/* Paragraph text — left align for readability */
...
.det-val {
    font-size: 9px; font-weight: 700; color: #0f2055;
    line-height: 1.2;
}
...
.qr-id {
    ...
    margin-top: 0.5mm; word-break: break-all;
}

/* ═══════════════════════════════════════════════════
   SECTION 3 — BOTTOM BAR  (161.5 → 183mm)
   NOTE: position:relative so float children work
═══════════════════════════════════════════════════ */
.bot-bar {
    position: absolute;
    top: 161.5mm; left: 0; right: 0; height: 22mm;
    z-index: 2;
}
/* Three equal columns via float */
.bb-col {
    float: left;
    width: 33.33%;
    height: 22mm;
...
/* ═══════════════════════════════════════════════════
   SECTION 4 — FOOTER  (183 → 207mm, dark navy)
═══════════════════════════════════════════════════ */
.footer {
    position: absolute;
    top: 183mm; left: 0; right: 0;
    height: 24mm; background: #0a1628; z-index: 2;
    text-align: center;
    padding-top: 5mm;
}
.footer-prop {
...
.footer-links {
    font-size: 7px; color: #7dd3fc;
    margin-top: 1.5mm; line-height: 1.5;
}

/* ═══════════════════════════════════════════════════
   SECTION 5 — COLORFUL BOTTOM STRIP  (207 → 210mm)
═══════════════════════════════════════════════════ */
.color-strip {
    position: absolute;
    top: 207mm; left: 0; right: 0;
    height: 3mm; z-index: 2;
    background: linear-gradient(to right,
        #0f2055 0%,
        #1a5fa8 10%,
        #0ea5e9 20%,
...
@php
    ...
    /* Duration — ensure "Hours" suffix */
    $rawDuration = $schedule?->duration ?? '';
    $duration    = $rawDuration
                    ? (preg_match('/\d/i', $rawDuration) && !preg_match('/hour/i', $rawDuration)
                        ? $rawDuration . ' Hours'
                        : $rawDuration)
                    : '—';
    ...
    $microTxt = str_repeat(
        'SUSTAINABLE MANAGEMENT SYSTEM INC. · SMS TRAINING CERTIFICATE · '.($enrollment->certificate_number ?? '').
        ' · VERIFIED · www.smscert.com/verify · ', 80);
@endphp
...
<div class="page">
    ...
    <div class="hdr">
        ...
        <div class="hdr-logo-panel">
            <div class="hdr-logo-img">
                @if($logo)<img src="{{ $logo }}">@else
                <div style="color:#fff;font-size:26px;font-weight:900;margin-top:4mm;">SMS</div>
                @endif
            </div>
        </div>
    ...
    <div class="det-box">
        ...
        <div class="det-row">
            <div class="det-lbl">Course Number</div>
        ...
        @if($isoTag)
        <div class="det-row">
            <div class="det-lbl">Training Standard</div>
        ...
        <div class="det-row">
            ...
            <div class="det-val">{{ $enrollment->certificate_number ?? 'N/A' }}</div>
        </div>
        <div class="det-sep"></div>
        ...
        <div class="det-sep"></div>
        <div class="det-row">
            <div class="det-lbl">IRQAO Registration ID</div>
            <div class="det-val">{{ $enrollment->irqao_reg_id }}</div>
        </div>
        @endif
    ...
    {{-- ══ BOTTOM BAR (float layout — no nested absolute) ══ --}}
    <div class="bot-bar">
        <div class="bb-clearfix">
            ...
            <div class="bb-col" style="padding-top:0.5mm;">
                ...
                <div class="logos-row">
                    @if($irqao)
                        <img src="{{ $irqao }}" class="third-logo">
                    @endif
                    @if($ascb)
                        <img src="{{ $ascb }}" class="third-logo">
                    @endif
                    @if(!$irqao && !$ascb)
    ...
    <div class="footer">
        <div class="footer-prop">
            This certificate is the property of Sustainable Management System Inc.
