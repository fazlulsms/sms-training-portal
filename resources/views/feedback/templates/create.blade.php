@extends('layouts.app')
@section('page-title', 'Create Feedback Template')
@section('content')
<style>
...
.q-card .q-header { display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; }
</style>
...
<div style="max-width:860px; margin:auto;">
    ...
    <div style="background:#fff; border-radius:14px; padding:28px; box-shadow:0 2px 12px rgba(0,0,0,.07);">
        <h2 style="font-size:20px; font-weight:800; color:#111827; margin:0 0 22px;">Create Feedback Template</h2>
        ...
        <form method="POST" action="{{ route('feedback.templates.store') }}">
            @csrf
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px;">
                <div class="fg" style="grid-column:1/-1;">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required placeholder="e.g. ILT Standard Evaluation">
                </div>
                <div class="fg">
            ...
            <div style="display:flex; gap:24px; flex-wrap:wrap; padding:14px 16px; background:#f8fafc; border-radius:8px; margin-bottom:22px;">
                ...
                <label style="display:flex; align-items:center; gap:8px; font-size:13px; font-weight:600; color:#374151; cursor:pointer;">
                    <input type="checkbox" name="require_for_certificate" value="1" {{ old('require_for_certificate') ? 'checked' : '' }}> Required for Certificate
                </label>
                <label style="display:flex; align-items:center; gap:8px; font-size:13px; font-weight:600; color:#374151; cursor:pointer;">
                    <input type="checkbox" name="allow_multiple" value="1" {{ old('allow_multiple') ? 'checked' : '' }}> Allow Multiple Submissions (Demo)
                </label>
            </div>
            ...
            <div id="questionsContainer">
                @php $defaultQuestions = old('questions', [
                    ['question_text'=>'Overall satisfaction with this training', 'question_type'=>'rating_5', 'category'=>'overall', 'is_required'=>1],
                    ['question_text'=>'Training content quality', 'question_type'=>'rating_5', 'category'=>'content', 'is_required'=>1],
                    ['question_text'=>'Trainer effectiveness', 'question_type'=>'rating_5', 'category'=>'trainer', 'is_required'=>1],
                    ['question_text'=>'Learning materials quality', 'question_type'=>'rating_5', 'category'=>'content', 'is_required'=>1],
                    ['question_text'=>'Practical examples and exercises', 'question_type'=>'rating_5', 'category'=>'content', 'is_required'=>0],
                    ['question_text'=>'Knowledge gained from this training', 'question_type'=>'rating_5', 'category'=>'overall', 'is_required'=>1],
                    ['question_text'=>'Would you recommend this training to colleagues?', 'question_type'=>'yes_no', 'category'=>'overall', 'is_required'=>1],
                    ['question_text'=>'What did you find most valuable?', 'question_type'=>'text', 'category'=>'open', 'is_required'=>0],
                    ['question_text'=>'What improvements would you suggest?', 'question_type'=>'text', 'category'=>'open', 'is_required'=>0],
                    ['question_text'=>'What topics would you like to learn next?', 'question_type'=>'text', 'category'=>'open', 'is_required'=>0],
                ]); @endphp
                ...
                <div class="q-card" id="qcard-{{ $i }}">
                    ...
                    <div style="display:grid; grid-template-columns:2fr 1fr 1fr; gap:10px; margin-bottom:10px;">
                        <input type="text" name="questions[{{ $i }}][question_text]" value="{{ $q['question_text'] }}" placeholder="Question text" required style="padding:8px 10px; border:1.5px solid #d1d5db; border-radius:6px; font-size:13px;">
            ...
            <div style="margin-top:24px; display:flex; gap:10px;">
                <button type="submit" style="background:linear-gradient(135deg,#1e3a8a,#2563eb); color:#fff; padding:12px 28px; border:none; border-radius:9px; font-weight:800; font-size:14px; cursor:pointer;">
                    Create Template
                </button>
                <a href="{{ route('feedback.templates.index') }}" style="background:#f1f5f9; color:#374151; padding:12px 20px; border-radius:9px; font-size:14px; font-weight:600; text-decoration:none;">Cancel</a>
            </div>
        </form>
    </div>
</div>
...
<script>
let qCount = {{ count($defaultQuestions) }};
...
function addQuestion() {
    ...
    card.innerHTML = `
        ...
        <label style="display:flex; align-items:center; gap:6px; font-size:12px; color:#374151; cursor:pointer;">
            <input type="hidden" name="questions[${i}][is_required]" value="0">
            <input type="checkbox" name="questions[${i}][is_required]" value="1" checked> Required
        </label>`;
