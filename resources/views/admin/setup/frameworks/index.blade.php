...
<div style="max-width:1100px; margin:auto;">
    ...
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:24px;">
        <div>
            <h2 style="font-size:22px; font-weight:800; color:#111827; margin:0 0 4px;">Learning Frameworks</h2>
            <div style="font-size:13px; color:#6b7280;">
                Layer 2 · LTF Taxonomy ·
        ...
        <a href="{{ route('setup.frameworks.create') }}"
    ...
    @include('admin.setup._tabs', ['active' => 'frameworks'])
        ...
        <table style="width:100%; border-collapse:collapse;">
            <thead>
                <tr style="background:#f8fafc; border-bottom:2px solid #e5e7eb;">
                    ...
                    <th style="padding:11px 16px; text-align:left; font-size:11px; font-weight:700; color:#6b7280; text-transform:uppercase; letter-spacing:.5px;">AI Hint</th>
                    <th style="padding:11px 16px; text-align:center; font-size:11px; font-weight:700; color:#6b7280; text-transform:uppercase; letter-spacing:.5px;">Days</th>
            ...
            <tbody>
                ...
                <tr style="border-bottom:1px solid #f3f4f6;">
                    <td style="padding:12px 16px;">
                        ...
                        @if($rec->description)
                        <div style="font-size:12px; color:#9ca3af; margin-top:2px; max-width:320px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">{{ $rec->description }}</div>
                        @endif
                    </td>
                    <td style="padding:12px 16px;">
                        @if($rec->ai_block_hint)
                        <code style="background:#f3f4f6; padding:2px 7px; border-radius:4px; font-size:12px; color:#374151;">{{ $rec->ai_block_hint }}</code>
                        @else
                        <span style="color:#d1d5db;">—</span>
                        @endif
                    </td>
                    <td style="padding:12px 16px; text-align:center; color:#6b7280;">{{ $rec->typical_duration_days ?? '—' }}</td>
                    ...
                    <td style="padding:12px 16px; text-align:center;">
                        <form method="POST" action="{{ route('setup.frameworks.toggle', $rec) }}" style="display:inline;">
                            @csrf @method('PATCH')
                    ...
                    <td style="padding:12px 16px; text-align:center; white-space:nowrap;">
                        <a href="{{ route('setup.frameworks.edit', $rec) }}" style="color:#1e3a8a; font-weight:600; font-size:13px; text-decoration:none; margin-right:12px;">Edit</a>
                        <form method="POST" action="{{ route('setup.frameworks.destroy', $rec) }}" style="display:inline;" onsubmit="return confirm('Delete {{ addslashes($rec->name) }}?')">
                            @csrf @method('DELETE')
                ...
                @empty
                <tr><td colspan="7" style="padding:40px; text-align:center; color:#9ca3af;">No frameworks yet. <a href="{{ route('setup.frameworks.create') }}" style="color:#1e3a8a;">Add the first one.</a></td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
