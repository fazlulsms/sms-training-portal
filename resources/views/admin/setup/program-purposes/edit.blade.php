...
<div style="max-width:680px; margin:auto;">
    ...
    <div style="background:#fff; border-radius:12px; box-shadow:0 2px 8px rgba(0,0,0,.06); padding:28px;">
        <h2 style="font-size:20px; font-weight:800; color:#111827; margin:0 0 6px;">Edit Program Purpose</h2>
        ...
        <form method="POST" action="{{ route('setup.program-purposes.update', $record) }}">
            @csrf @method('PUT')
            ...
            <div style="margin-bottom:16px;">
                ...
                <select name="suggested_framework_id" style="width:100%; padding:10px 13px; border:1.5px solid #d1d5db; border-radius:8px; font-size:14px; font-family:inherit; box-sizing:border-box;">
                    ...
                    <option value="{{ $id }}" {{ old('suggested_framework_id', $record->suggested_framework_id) == $id ? 'selected' : '' }}>{{ $name }}</option>
                    @endforeach
                </select>
