...
<div style="max-width:680px; margin:auto;">
    <div style="margin-bottom:20px;">
        <a href="{{ route('setup.delivery-methods.index') }}" style="color:#6b7280; font-size:13px; text-decoration:none;">← Delivery Methods</a>
    </div>
    <div style="background:#fff; border-radius:12px; box-shadow:0 2px 8px rgba(0,0,0,.06); padding:28px;">
        <h2 style="font-size:20px; font-weight:800; color:#111827; margin:0 0 24px;">Edit Delivery Method</h2>
        ...
        <form method="POST" action="{{ route('setup.delivery-methods.update', $record) }}">
            @csrf @method('PUT')
            ...
            <div style="margin-bottom:16px;">
                ...
                <textarea name="description" rows="3"
                          style="width:100%; padding:10px 13px; border:1.5px solid #d1d5db; border-radius:8px; font-size:14px; font-family:inherit; box-sizing:border-box; resize:vertical;">{{ old('description', $record->description) }}</textarea>
            </div>
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-bottom:24px;">
                <div>
                    ...
                    <input type="number" name="display_order" value="{{ old('display_order', $record->display_order) }}" min="0"
                           style="width:100%; padding:10px 13px; border:1.5px solid #d1d5db; border-radius:8px; font-size:14px; font-family:inherit; box-sizing:border-box;">
                </div>
                <div>
            ...
            <div style="display:flex; gap:12px;">
                <button type="submit" style="background:#1e3a8a; color:#fff; padding:10px 24px; border:none; border-radius:8px; font-weight:700; font-size:14px; cursor:pointer;">Update</button>
                <a href="{{ route('setup.delivery-methods.index') }}" style="padding:10px 20px; border:1.5px solid #d1d5db; border-radius:8px; font-weight:600; font-size:14px; color:#374151; text-decoration:none;">Cancel</a>
            </div>
        </form>
    </div>
</div>
