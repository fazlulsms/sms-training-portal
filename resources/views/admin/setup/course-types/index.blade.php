...
<div style="max-width:1100px; margin:auto;">
    ...
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:24px;">
        <div>
            <h2 style="font-size:22px; font-weight:800; color:#111827; margin:0 0 4px;">Course Types</h2>
            <div style="font-size:13px; color:#6b7280;">
                Layer 1 · LTF Taxonomy ·
        ...
        <a href="{{ route('setup.course-types.create') }}"
    ...
    {{-- Navigation tabs --}}
    @include('admin.setup._tabs', ['active' => 'course-types'])
        ...
        <table style="width:100%; border-collapse:collapse;">
            <thead>
                <tr style="background:#f8fafc; border-bottom:2px solid #e5e7eb;">
                    <th style="padding:11px 16px; text-align:left; font-size:11px; font-weight:700; color:#6b7280; text-transform:uppercase; letter-spacing:.5px;">#</th>
                    ...
                    <th style="padding:11px 16px; text-align:left; font-size:11px; font-weight:700; color:#6b7280; text-transform:uppercase; letter-spacing:.5px;">Group</th>
            ...
            <tbody>
                ...
                <tr style="border-bottom:1px solid #f3f4f6;">
                    <td style="padding:12px 16px; color:#9ca3af; font-size:13px;">{{ $rec->display_order }}</td>
                    <td style="padding:12px 16px; font-weight:600; color:#111827;">{{ $rec->name }}</td>
                    <td style="padding:12px 16px;">
                        @php $groupColors = ['elearning'=>['#dbeafe','#1e40af'], 'ilt'=>['#fef3c7','#92400e'], 'assessment'=>['#f3e8ff','#6b21a8']]; $gc = $groupColors[$rec->group] ?? ['#f3f4f6','#6b7280']; @endphp
                        <span style="background:{{ $gc[0] }}; color:{{ $gc[1] }}; padding:2px 10px; border-radius:20px; font-size:12px; font-weight:700;">
                            {{ \App\Models\LtfCourseType::GROUPS[$rec->group] ?? $rec->group }}
                        </span>
                    </td>
                    ...
                    <td style="padding:12px 16px; text-align:center;">
                        <form method="POST" action="{{ route('setup.course-types.toggle', $rec) }}" style="display:inline;">
                            @csrf @method('PATCH')
                    ...
                    <td style="padding:12px 16px; text-align:center; white-space:nowrap;">
                        <a href="{{ route('setup.course-types.edit', $rec) }}" style="color:#1e3a8a; font-weight:600; font-size:13px; text-decoration:none; margin-right:12px;">Edit</a>
                        <form method="POST" action="{{ route('setup.course-types.destroy', $rec) }}" style="display:inline;" onsubmit="return confirm('Delete {{ addslashes($rec->name) }}?')">
                            @csrf @method('DELETE')
                ...
                @empty
                <tr><td colspan="7" style="padding:40px; text-align:center; color:#9ca3af;">No course types yet. <a href="{{ route('setup.course-types.create') }}" style="color:#1e3a8a;">Add the first one.</a></td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
