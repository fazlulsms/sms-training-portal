...
<div style="margin-bottom:18px;">
    ...
    <input name="name" value="{{ $v('name') }}" required
           ...
           placeholder="e.g. Self-Paced eLearning">
</div>

<div style="margin-bottom:18px;">
    <label style="display:block; font-size:13px; font-weight:700; color:#374151; margin-bottom:6px;">Group <span style="color:#dc2626;">*</span></label>
    <select name="group" required style="width:100%; padding:9px 12px; border:1px solid #d1d5db; border-radius:8px; font-size:14px; box-sizing:border-box; background:#fff;">
        <option value="">— select group —</option>
        @foreach($groups as $key => $label)
        <option value="{{ $key }}" {{ $v('group') === $key ? 'selected' : '' }}>{{ $label }}</option>
        @endforeach
    </select>
</div>
